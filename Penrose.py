...
class Penrose(object):
	...
	def compute_pov_objs(self,**config):
		...
		for rhombi,color in self.tile():
			p1,p2,p3,p4 = rhombi
			polygon = Polygon(5,p1,p2,p3,p4,p1,Texture(Pigment('color',color),config['default']))
			objects_pool.append(polygon)

		for p,q in zip(rhombi,[p2,p3,p4,p1]):
			cylinder = Cylinder(p,1,config['edge_thickness'],config['vertex_texture'])
	...
	def rhombus(self,r,s,kr,ks):
		...
		point = (Penrose.GRIDS[r]*(ks - self.shift[s]) - Penrose.GRIDS[s]*(kr - self.shift[r]))*1j\
			/Penrose.GRIDS[s-r].imag
